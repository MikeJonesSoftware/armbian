...
function pre_customize_image__set_waveshare_values { 
	echo "Configuring waveshare-3566-specific values..."
...
EOF
	...
	export ETHERNET_INTERFACE=\$(/usr/bin/ls /sys/class/net | /usr/bin/grep -v 'lo' | /usr/bin/head -n 1)
	echo "Detected ethernet interface: \${ETHERNET_INTERFACE}"

	echo "Installing tcpdump..."
	apt install tcpdump -y -qq
...
CPUEOF
	...
	# Create a bash script that requests a refresh on the _ssh._tcp service type.
	# This is used to prevent the switch from pruning the _ssh._tcp service type
	cat > /usr/local/bin/mdns-pulse.sh << PULSEEOF
#!/bin/bash
# Force a fresh process environment to trigger a real wire-level mDNS probe
#/bin/sh -c "/usr/bin/avahi-browse -rt _ssh._tcp --terminate" > /dev/null 2>&1
...
<service-group>
	...
	<service>
		<type>_ssh._tcp</type>
		<port>25000</port>
		<txt-record>nickname=Initialized Waveshare Core3566 Device</txt-record>
	</service>
</service-group>
AVAHIEOF
